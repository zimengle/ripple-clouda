var deviceSettings = ripple('deviceSettings'),
    ...
    event = ripple('event'),
    mediaModule = ripple('platform/clouda/media'),
...
_self = module.exports = {
    ...
    media:{
        ...
        operateMedia:function(link, operator, options){
            var media = mediaObj[link];

            if(media == null){
                media = new Media(link);
                mediaObj[link] = media;
            }
            if("getDuration" == operator){
                media.onloadedmetadata(function(){
                    options && options.onsuccess(media.getDuration());
                });

            }else if("getCurrentPosition" == operator){
                 media.onloadedmetadata(function(){
                    options && options.onsuccess(media.getCurrentPosition());
                });
            }else if(/play|pause|stop/.test(operator)){
                debugger;
                media.onloadedmetadata(function(){
                    media[operator]();
                });
